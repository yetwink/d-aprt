...
function plugins() {
  ...
  gsapWithCSS.registerPlugin(ScrollTrigger_ScrollTrigger, ScrollToPlugin, DrawSVGPlugin);
...
function popups() {
  ...
  const mediaMin768px = window.matchMedia("(min-width: 768px)").matches;
  // //Prices Popup(wp-popup)
  // let wpPopup = document.querySelector('.wp-popup');
  // let wpPopupExit = document.querySelector('.wp-popup__exit');
  // let wpPopupBtn = document.querySelector('#wpPopupBtn');
  ...
  // TimePopup START
  let tpPopupBtns = document.querySelectorAll('#tpPopupBtn');
  let tpPopup = document.querySelector('.timepopup');
  let tpPopupWrap = tpPopup.querySelector('.popup-tp__wrapper');
  let tpPopupExit = document.querySelector('.timepopup__close');

  // tpPopupBtns.forEach((tpPopupBtn) => {
  //   tpPopupBtn.addEventListener('click', () => {
  //     tpPopup.classList.add('active');
  //     lenisScroll.stop();
  //     if (queryMatches(769, 'min')) {
  //       setTimeout(() => {
  //         tpPopup.classList.add('step-2');
  //         tpPopupWrap.style.overflow = 'visible';
  //       }, 700);
  //     }
  //   });
  // });
  ...
  const allBtnsPop = document.querySelectorAll('.buttons');
  ...
  tpPopupExit.addEventListener('click', () => {
    if (mediaMin768px) {
      setTimeout(() => {
        tpPopup.classList.remove('active');
        document.body.classList.remove('hide-wp');
      }, 700);
    } else {
      tpPopup.classList.remove('active');
    ...
    tpPopup.classList.remove('step-2');
    tpPopupWrap.style.overflow = 'hidden';
    lenisScroll.start();
  });

  tpPopup.addEventListener('click', (e) => {
    if (e.target.closest('.popup-tp__wrapper')) {
      return;
    }
    if (mediaMin768px) {
      setTimeout(() => {
  ...
  // Main START
  let PopupBtns = document.querySelectorAll('#popupBtn');
  let Popup = document.querySelector('.popup');
  let PopupExit = document.querySelector('.popup-test__exit');
  let PopupWrap = Popup.querySelector('.popup-tp__wrapper');
    ...
    PopupBtn.addEventListener('click', () => {
      ...
      if (queryMatches(769, 'min')) {
        setTimeout(() => {
          ...
          PopupWrap.style.overflow = 'visible';
        }, 700);
      }
    });
  });
  ...
  //villas popup location
  let PopupLcBtns = document.querySelectorAll('.popup-lc__btn');
  let PopupLc = document.querySelector('.popup-lc');
  let PopupLcExit = document.querySelector('.popup-lc__close');
  let PopupLcWrap = PopupLc.querySelector('.popup-tp__wrapper');
  PopupLcBtns.forEach((PopupLcBtn) => {
    PopupLcBtn.addEventListener('click', () => {
      PopupLc.classList.add('active');
      lenisScroll.stop();
      if (queryMatches(769, 'min')) {
        setTimeout(() => {
          PopupLc.classList.add('step-2');
          PopupLcWrap.style.overflow = 'visible'
        }, 700);
      }
    });
  });
  PopupLcExit.addEventListener('click', () => {
    if (mediaMin768px) {
      setTimeout(() => {
        PopupLc.classList.remove('active');
        document.body.classList.remove('hide-wp');
      }, 700);
    } else {
      PopupLc.classList.remove('active');
    ...
    PopupLc.classList.remove('step-2');
    lenisScroll.start();
    PopupLcWrap.style.overflow = 'hidden'
  });
  PopupLc.addEventListener('click', (e) => {
    if (e.target.closest('.popup-tp__wrapper')) {
      return;
    }
    if (mediaMin768px) {
...
let dbPopup = document.querySelector('.popup-db');
let dbPopupExit = document.querySelector('.popup-db__exit');
let dbPopupBtn = document.querySelector('#popupDb');
let dbPopupWrapper = document.querySelector('.popup-db__wrapper');

// dbPopupBtn.addEventListener('click', () => {
//   dbPopup.classList.add('active');
//   lenisScroll.stop();

//   setTimeout(() => {
//     dbPopup.classList.add('step-2');
//     // tpPopupWrap.style.overflow = 'visible';
//   }, 700);
// });

// dbPopupExit.addEventListener('click', () => {
//   dbPopup.classList.remove('active');
//   document.body.classList.remove('hide-wp');
//   lenisScroll.start();
// });
...
dbPopupBtn.addEventListener('click', () => {
  ...
  if (queryMatches(769, 'min')) {
    setTimeout(() => {
      dbPopup.classList.add('step-2');
      dbPopupWrapper.style.overflow = 'visible'
    }, 700);
  }
});
...
dbPopupExit.addEventListener('click', () => {
  ...
  dbPopup.classList.remove('step-2');
  lenisScroll.start();
  dbPopupWrapper.style.overflow = 'hidden';
});
...
function header() {
  let m991 = window.matchMedia("(max-width: 991px)").matches;
  ...
  if (m991) {
    ...
    anchorLinks.forEach((link) => {
      link.addEventListener("click", () => {
        const linkDataId = link.getAttribute("data-link");
        const matchSectionId = document.querySelector(`.${linkDataId}`);
        gsap.to(window, {
          duration: 1,
          scrollTo: {
            y: matchSectionId,
...
function intro() {
  ...
  if (queryMatches(1200)) {
    let fixedWp = document.querySelector('.fixed-whatsapp');
    ...
    let wpTL = gsap.timeline({
      scrollTrigger: {
        ...
        // onEnter: ()=>{
        //   fixedWp.style.opacity = '0';
        // },
        onEnterBack: ()=>{
          fixedWp.style.opacity = '0';
        },
        onLeave: ()=>{
          fixedWp.style.opacity = '1';
        },
      }
    })
  }
  ...
  if (queryMatches(768)) {
    document.querySelectorAll(".intro__form input").forEach((input) => {
...
;// CONCATENATED MODULE: ./src/js/sections/villas.js

function villas() {
  const mediaMax768px = window.matchMedia("(max-width: 768px)").matches;
  const autoPlayDelayOption = mediaMax768px ? 10000 : 5000;
  gsap.set('.villas__desc', { clipPath: 'inset(0 0 100% 0)' });
  const tl = gsap.timeline({
    scrollTrigger: {
      trigger: '.villas',
  ...
  tl.to('.villas__desc', { clipPath: 'inset(0 0 0% 0)', duration: 1 });
  var swiper = new Swiper('.villas__cards', {
    slidesPerView: 1,
    centeredSlides: true,
    spaceBetween: 30,
    loop: true,
    ...
    on: {
      ...
      init: () => {
        const swiperContainer = document.querySelector('.villas__cards');
...
;// CONCATENATED MODULE: ./src/js/sections/dubai.js
function dubai() {
    let m768 = window.matchMedia("(max-width: 768px)").matches;

    if(m768){
        const element = document.querySelector('.dubai__wrapper')
        ...
        const cards = document.querySelectorAll('.dubai__card');
        let activeCard = document.querySelector('.dubai__card.mob-active');
    
        cards.forEach((el, i)=>{
            el.addEventListener('click', ()=>{
    
                if(el.classList.contains('mob-active')){
                    el.classList.remove('mob-active');
                    activeCard?.classList.remove('mob-active')
                } else {
                    activeCard?.classList.remove('mob-active')
        ...
        //     if(!event.target.closest('.dubai__cards')){
        //         activeCard?.classList.remove('mob-active')
        //     }
        // })
    }
    else {
        const dubaiBg = document.querySelector('.dubai__bg'),
            dubaiTitle = document.querySelector('.dubai__title');
        ...
        let titleTL = gsap.timeline({
            scrollTrigger: {
                ...
                once: true,
                onEnter: ()=>{
                    dubaiBg.classList.add('active')
                    dubaiTitle.style.opacity = '1';
                }
            }
        })
...
;// CONCATENATED MODULE: ./src/js/sections/feedback.js
function feedback() {
  const media768px = matchMedia("(max-width: 768px)").matches;
  const feedbackSection = document.querySelector(".feedback");
  const feedbackMobMain = document.querySelectorAll(".feedback-main");
  const feedbackMobBtnOpen = document.querySelectorAll(
    ".feedback-footer__subtitle.mob-open"
  );
  const feedbackMobBtnClose = document.querySelectorAll(
    ".feedback-footer__subtitle.mob-close"
  );
  ...
  if (media768px) {
    feedbackMobMain.forEach((block, index) => {
      console.log(block.offsetHeight);
      if (block.offsetHeight > 265) {
        feedbackMobMain[index].classList.add("_scroll");
      }
    });

    feedbackMobBtnOpen.forEach((open, index) => {
      open.addEventListener("click", () => {
        feedbackMobMain.forEach((block) => block.classList.remove("_show"));
        feedbackMobMain[index].classList.add("_show");
      });
    });

    feedbackMobBtnClose.forEach((close, index) => {
      close.addEventListener("click", () => {
        feedbackMobMain[index].classList.remove("_show");
      });
    });
  }

  const avatarBgColor = document.querySelectorAll(".feedback-header__avatar");
  const avatarInitial = document.querySelectorAll(".feedback-header__initial");
  const avatarUserName = document.querySelectorAll(
    ".feedback-header-user__name"
  );
  const avatarImg = document.querySelectorAll(".feedback-header__avatar img");
  const currentUrl = window.location.href;
  let random = 0;

  avatarImg.forEach((image, index) => {
    if (image.src === currentUrl || image.src === "") {
      image.remove();
    }
    random = Math.floor(Math.random() * 10);
    if (random < 3) {
      avatarBgColor[index].style.backgroundColor = "rgba(216, 27, 96, 1)";
    } else if (random < 6) {
      avatarBgColor[index].style.backgroundColor = "rgba(242, 153, 0, 1)";
    } else {
      avatarBgColor[index].style.backgroundColor = "rgba(2, 119, 189, 1)";
    }
  });

  avatarBgColor.forEach((bgColor) => {
    const image = bgColor.querySelector("img");
    if (image !== null) {
      bgColor.style.backgroundColor = "rgba(255, 255, 255, 1)";
    }
  });

  avatarInitial.forEach((initial, index) => {
    const name = avatarUserName[index].textContent.slice(0, 1).toUpperCase();
    initial.textContent = name;
  });
}

;// CONCATENATED MODULE: ./src/js/sections/agency.js
function agency() {
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (async function loading() {
    ...
    const circle = document.querySelector(
      '.progres__ring-anim .progres__ring-circle'
    );
      ...
      if (loadedMedia === totalMedia) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            introBlock.classList.add('step-2');
            setTimeout(() => {
              ...
              itiBtn.forEach((btn) => {
                btn.addEventListener('click', () => {
                  setTimeout(() => {
                    const itiListmob =
                      document.querySelector('.iti--container');
